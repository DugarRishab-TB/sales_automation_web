  ...
  return (
    <div className="page-container">
      <div className="page-header">
        <div>
          <Title level={2} style={{ margin: 0 }}>{title} Details</Title>
          <Text type="secondary">ID: {id}</Text>
        </div>
        <Space>
          <Button icon={<EditOutlined />} type="primary" onClick={onEdit}>Edit</Button>
          <Button icon={<DeleteOutlined />} danger onClick={onDelete}>Delete</Button>
      ...
      <Card className="page-content" loading={loading}>
        {record && mode === 'leads' && (
          <Descriptions bordered column={1} size="middle">
            ...
            <Descriptions.Item label="Status">{record.status ? <Tag>{record.status}</Tag> : '-'}</Descriptions.Item>
            ...
            <Descriptions.Item label="LinkedIn">{record.linkedin || '-'}</Descriptions.Item>
            <Descriptions.Item label="Job Title">{record.jobTitle || '-'}</Descriptions.Item>
            <Descriptions.Item label="Sales Team">{record.salesTeam?.name || record.salesTeamId || '-'}</Descriptions.Item>
            <Descriptions.Item label="Notes">{record.notes || '-'}</Descriptions.Item>
            <Descriptions.Item label="Created At">{record.createdAt ? new Date(record.createdAt).toLocaleString() : '-'}</Descriptions.Item>
            <Descriptions.Item label="Updated At">{record.updatedAt ? new Date(record.updatedAt).toLocaleString() : '-'}</Descriptions.Item>
          </Descriptions>
        )}
        ...
        {record && mode === 'emails' && (
          <Descriptions bordered column={1} size="middle">
            ...
            <Descriptions.Item label="To">{record.toEmail || '-'}</Descriptions.Item>
            <Descriptions.Item label="From">{record.fromEmail}</Descriptions.Item>
            <Descriptions.Item label="Status">{record.status ? <Tag>{record.status}</Tag> : '-'}</Descriptions.Item>
            <Descriptions.Item label="Lead">{record.lead?.name || record.leadId || '-'}</Descriptions.Item>
            <Descriptions.Item label="Sales Team">{record.salesTeam?.name || record.salesTeamId || '-'}</Descriptions.Item>
            <Descriptions.Item label="Open Count">{record.openCount}</Descriptions.Item>
            <Descriptions.Item label="Click Count">{record.clickCount}</Descriptions.Item>
            <Descriptions.Item label="Sent At">{record.sentAt ? new Date(record.sentAt).toLocaleString() : '-'}</Descriptions.Item>
            <Descriptions.Item label="Opened At">{record.openedAt ? new Date(record.openedAt).toLocaleString() : '-'}</Descriptions.Item>
            <Descriptions.Item label="Clicked At">{record.clickedAt ? new Date(record.clickedAt).toLocaleString() : '-'}</Descriptions.Item>
            <Descriptions.Item label="Created At">{record.createdAt ? new Date(record.createdAt).toLocaleString() : '-'}</Descriptions.Item>
            <Descriptions.Item label="Updated At">{record.updatedAt ? new Date(record.updatedAt).toLocaleString() : '-'}</Descriptions.Item>
        ...
        {record && mode === 'sales' && (
          <Descriptions bordered column={1} size="middle">
            ...
            <Descriptions.Item label="Password">{record.password || '-'}</Descriptions.Item>
            <Descriptions.Item label="Created At">{record.createdAt ? new Date(record.createdAt).toLocaleString() : '-'}</Descriptions.Item>
            <Descriptions.Item label="Updated At">{record.updatedAt ? new Date(record.updatedAt).toLocaleString() : '-'}</Descriptions.Item>
          </Descriptions>
        )}
      ...
      <Modal
        ...
        destroyOnClose
      >
        <Form form={form} layout="vertical" onFinish={onUpdate} preserve={false}>
          {formFields.map((f) => (
